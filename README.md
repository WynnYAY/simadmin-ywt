...
SimAdmin VoLTE / Wi-Fi Calling **发行仓库** — 提供设备一键安装脚本与 OTA 发布包。
...
- Releases — 各架构 OTA 发布包（`simadmin[-wfc]-{aarch64,armv7,x86_64}.tar.gz`）
...
```sh
sh install_latest.sh --wfc
```

安装指定版本：

```sh
sh install_latest.sh -v1.1.39 --wfc
```
...
| `WFC=1` / `VARIANT=wfc` | — | 安装 Wi-Fi Calling 变体 |
