...
REPO="${REPO:-WynnYAY/simadmin-ywt}"
...
usage() {
  printf '%s\n' \
    ...
    '  --service-name NAME    Main service name (default: simadmin)' \
    ...
    '  SIMADMIN_VERIFY_ASSET=auto (auto|1|0; verify release SHA-256 via GitHub API)' \
    '  SIMADMIN_ASSET_SHA256=... (pin an expected digest for the release asset)' \
    '  SIMADMIN_SKIP_HEALTHCHECK=0 (set to 1 to skip the post-install health check)' \
    '  GH_PROXY=https://gh-proxy.com/' \
    '' \
    'Platform notes:' \
    '  Debian/Ubuntu installs use systemd + ModemManager/NetworkManager.' \
    '  OpenWrt (/etc/openwrt_release present) installs use a procd init script,' \
    '  best-effort opkg dependencies, and skip NetworkManager/udev.'
}
...
detect_platform() {
  if [ -f /etc/openwrt_release ]; then
    IS_OPENWRT=1
    openwrt_release="$(grep DISTRIB_RELEASE /etc/openwrt_release 2>/dev/null | cut -d"'" -f2 || true)"
    echo "==> detected OpenWrt system (release: ${openwrt_release:-unknown})"
    echo "    systemd service management is unavailable; using procd init script"
  fi
}
...
fetch_to_file() {
  ...
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$fetch_url" -o "$fetch_dst"
  elif command -v uclient-fetch >/dev/null 2>&1; then
    echo "    (using uclient-fetch fallback)" >&2
    uclient-fetch -O "$fetch_dst" "$fetch_url"
  else
    wget -q -O "$fetch_dst" "$fetch_url"
  fi
}
...
fetch_to_stdout() {
  ...
  if command -v curl >/dev/null 2>&1; then
    ...
    echo "    (using uclient-fetch fallback)" >&2
    uclient-fetch -O - "$fetch_url"
  else
    wget -q -O - "$fetch_url"
  fi
}

simadmin_service_is_active() {
  if [ "$IS_OPENWRT" = "1" ]; then
    if command -v ubus >/dev/null 2>&1 \
      && ubus call service list 2>/dev/null | grep -q "\"${SERVICE_NAME}\""; then
      return 0
    fi
    pgrep -f "${INSTALL_DIR}/simadmin" >/dev/null 2>&1
    return $?
  fi
  systemctl is-active --quiet "${SERVICE_NAME}.service"
}

stop_simadmin_service() {
  if [ "$IS_OPENWRT" = "1" ]; then
...
print_simadmin_status() {
  if [ "$IS_OPENWRT" = "1" ]; then
    if command -v ubus >/dev/null 2>&1; then
      ubus call service list 2>/dev/null | grep -A4 "\"${SERVICE_NAME}\"" || true
    fi
    return 0
  fi
...
wait_for_simadmin_health() {
  ...
  if ! command -v curl >/dev/null 2>&1; then
    echo "warning: curl unavailable; checking service state only" >&2
    simadmin_service_is_active
    return $?
  fi
  health_attempt=0
...
install_file_mode() {
  ...
  install_dst="$2"
  install_perm="$3"
  cp -f "$install_src" "$install_dst" 2>/dev/null || { rm -f "$install_dst"; cp "$install_src" "$install_dst"; }
  chmod "$install_perm" "$install_dst"
}
...
verify_release_asset() {
  ...
  if [ -z "$expected_sha" ] && [ -z "$ASSET_URL" ]; then
    ...
    if [ "$VERSION" = "latest" ]; then
      digest_api="https://api.github.com/repos/${REPO}/releases/latest"
    else
      digest_api="https://api.github.com/repos/${REPO}/releases/tags/$(version_to_tag "$VERSION")"
    fi
    release_json="$(read_with_proxies "$digest_api" 2>/dev/null || true)"
  ...
  if [ -z "$expected_sha" ]; then
    if [ "$SIMADMIN_VERIFY_ASSET" = "auto" ]; then
      echo "warning: no trusted SHA-256 digest found for ${verify_asset_name}; skipping verification" >&2
      return 0
    fi
...
install_system_dependencies() {
  ...
  if [ "$IS_OPENWRT" = "1" ]; then
    install_system_dependencies_openwrt
    return $?
  fi
...
install_system_dependencies_openwrt() {
  if ! command -v opkg >/dev/null 2>&1; then
    echo "warning: opkg is unavailable; ensure ModemManager (mmcli), libqmi (qmicli) and curl are installed manually" >&2
    return 0
  fi

  echo "==> installing OpenWrt runtime dependencies (best-effort)"
  if ! opkg update >/dev/null 2>&1; then
    echo "warning: opkg update failed; continuing with cached package lists" >&2
  fi

  for pkg in ca-bundle curl tar; do
    if ! opkg list-installed 2>/dev/null | grep -q "^${pkg} "; then
      if ! opkg install "$pkg" >/dev/null 2>&1; then
        echo "warning: failed to install OpenWrt package: ${pkg}" >&2
      fi
    fi
  done

  # ModemManager + QMI/MBIM stack. On some devices (e.g. msm8916-based modems)
  # the QMI kernel drivers are built in and USB kmods are unnecessary; failures
  # here are non-fatal because the fleet device may already provide these tools.
  for pkg in modemmanager libqmi libmbim kmod-usb-net-qmi-wwan; do
    if ! opkg list-installed 2>/dev/null | grep -q "^${pkg} "; then
      if ! opkg install "$pkg" >/dev/null 2>&1; then
        echo "note: optional OpenWrt package not installed: ${pkg}" >&2
      fi
    fi
  done
}
...
start_runtime_services() {
  if [ "$IS_OPENWRT" = "1" ]; then
    for mm_init in /etc/init.d/modemmanager /etc/init.d/ModemManager /etc/init.d/modem-manager; do
      if [ -x "$mm_init" ]; then
        echo "==> enabling ModemManager via procd ($mm_init)"
        "$mm_init" enable >/dev/null 2>&1 || true
        "$mm_init" start >/dev/null 2>&1 || true
        return 0
      fi
    done
    echo "warning: ModemManager init script not found on OpenWrt; mmcli-dependent features may be unavailable" >&2
    return 0
  fi
...
refresh_modem_devices() {
  ...
  if [ "$IS_OPENWRT" = "1" ]; then
    # OpenWrt has no udev; hotplug picks up modem devices automatically.
    for mm_init in /etc/init.d/modemmanager /etc/init.d/ModemManager /etc/init.d/modem-manager; do
      if [ -x "$mm_init" ]; then
        echo "==> restarting ModemManager via procd ($mm_init)"
        "$mm_init" restart >/dev/null 2>&1 || true
        return 0
      fi
    done
    return 0
  ...
  case "$src_url" in
    https://github.com/*|https://raw.githubusercontent.com/*|https://objects.githubusercontent.com/*|https://api.github.com/*)
      for proxy in $GH_PROXY $GH_PROXY_FALLBACKS ""; do
        url="${proxy}${src_url}"
        echo "    ${url}"
        if fetch_to_file "$url" "$dst_path"; then
          return 0
        fi
    ...
    *)
      echo "    ${src_url}"
      fetch_to_file "$src_url" "$dst_path"
      return $?
      ;;
  esac
...
install_openwrt_init_script() {
  init_dst="/etc/init.d/${SERVICE_NAME}"
  echo "==> installing OpenWrt procd init script (${init_dst})"
  mkdir -p /etc/init.d
  cat > "$init_dst" <<EOF
...
install_modem_recovery_service() {
  if [ "$IS_OPENWRT" = "1" ]; then
    echo "==> skipping modem recovery systemd unit (systemd not available on OpenWrt)"
    return 0
  fi
...
main() {
  ...
  install_system_dependencies
  if ! command -v curl >/dev/null 2>&1; then
    if [ "$IS_OPENWRT" = "1" ] && command -v uclient-fetch >/dev/null 2>&1; then
      echo "warning: curl unavailable; falling back to uclient-fetch for downloads"
    else
      echo "error: missing required command: curl" >&2
      exit 1
    fi
  ...
  verify_release_asset "$archive_path" "$(resolve_simadmin_asset_name)"
  ...
  mkdir -p "${INSTALL_DIR}"
  install_file_mode "${tmp_dir}/pkg/simadmin" "${INSTALL_DIR}/simadmin" 0755
  ...
  if [ -f "${tmp_dir}/pkg/meta.json" ]; then
    install_file_mode "${tmp_dir}/pkg/meta.json" "${INSTALL_DIR}/meta.json" 0644
...
EOF
  ...
  if [ "$IS_OPENWRT" = "1" ]; then
    echo "==> installing OpenWrt procd init script"
  else
  ...
  wait_for_simadmin_health || true

  echo "==> done"
  if [ "$IS_OPENWRT" = "1" ]; then
    echo "    service: /etc/init.d/${SERVICE_NAME} (procd)"
    echo "    modem recovery: skipped on OpenWrt (systemd not available)"
  else
