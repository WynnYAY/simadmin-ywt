...
restart_modem_remoteproc() {
  # MSM8916 等平台上 modem DSP 的 remoteproc 节点名是 <addr>.remoteproc（例如
  # 4080000.remoteproc），name 里没有 mss/modem 字样，只有 firmware（mba.mbn）
  # 能区分它和 wcnss/pronto/adsp，因此固件名必须参与匹配。
  for r in /sys/class/remoteproc/remoteproc*; do
    [ -e "$r/name" ] || continue
    name="$(cat "$r/name" 2>/dev/null)"
    fw="$(cat "$r/firmware" 2>/dev/null)"
    if printf '%s %s\n' "$name" "$fw" | grep -qi "wcnss\|pronto\|adsp\|venus"; then
      continue
    fi
    if printf '%s %s\n' "$name" "$fw" | grep -qi "mss\|modem\|mba"; then
      echo stop > "$r/state"
      sleep 3
      echo start > "$r/state"
      log "DSP remoteproc restarted: ${r} (name=${name} firmware=${fw})"
      return 0
    fi
  done
